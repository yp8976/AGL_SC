...
def utils2(x,y,p=2):
    ...
    wasserstein_distance = ops.pow(ops.sum(ops.pow(wasserstein_distance, p), dim=1), 1.0 / p)
...
def BPR_train_original(dataset, recommend_model, loss_class, W, neg_k=1, w=None):
    ...
    for (batch_users,
          batch_pos,
          batch_neg) in tqdm(utils.minibatch(users,
                                                   posItems,
                                                   negItems,
                                                   batch_size=world.config['bpr_batch_size'])):
        cri = bpr.stageOne(batch_users, batch_pos, batch_neg)
        aver_loss += 0.5* utils2(W,ops.mm(Recmodel.embedding_user.weight,Recmodel.embedding_item.weight.t()))+cri
...
def Test(dataset, Recmodel, epoch, w=None, multicore=0):
    ...
    results = {'ndcg': np.zeros(len(world.topks)),
